...
public class Sql2oCandidateRepository implements CandidateRepository {
    ...
    public Candidate save(Candidate candidate) {
        try (var connection = sql2o.open()) {
            var sql = """
                      INSERT INTO candidates(name, description, city_id, creation_date, file_id)
                      VALUES (:name, :description, :cityId, :creationDate, :fileId)
                      """;
    ...
    public boolean update(Candidate candidate) {
        try (var connection = sql2o.open()) {
            var sql = """
                    UPDATE candidates
                    SET name = :name, description = :description, city_id = :cityId,
